...
> `/home/user/gsd-cards/.claude/gsd-core/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "/home/user/gsd-cards/.claude/gsd-core/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @/home/user/gsd-cards/.claude/agents/gsd-advisor-researcher.md for your role and instructions.
